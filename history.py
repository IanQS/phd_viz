...
def _(life_details, pd, px):
    ...
    def plot_timeline_grouped_resource(events: pd.DataFrame):
        fig = px.timeline(events, x_start="Start", x_end="Finish", y="Resource", color="Resource", text="Event", opacity=0.8)
...
def _(datetime, ff, pd):
    ...
    def plot_timeline_grouped_project(events: list[dict[str, str]]):
        ...
        for i, event in enumerate(events):
            ...
            annots.append(
                dict(x=midpoint,y=y_idx,text=event["Resource"], showarrow=False, font=dict(color='black'))
            )

        fig = ff.create_gantt(events, index_col='Resource', show_colorbar=True,
                              group_tasks=True, title="Research Projects", show_hover_fill=True,
                             )
